program test_ncdr_attrs
    ...
    call nc_diag_read_get_global_attr_names(tmp_ncdr_id_2, nattrs, nattrs_len, attr_names)
    ...
    do i = 1, nattrs
        attr_name = trim(attr_names(i))
        call nc_diag_read_assert_global_attr(tmp_ncdr_id_2, attr_name, attr_type, attr_len)
        if (nc_diag_read_check_global_attr(tmp_ncdr_id_2, attr_name) == .FALSE.) &
            call error("Can't find attr with check(), even when it's listed!")
        call nc_diag_read_get_global_attr_len(tmp_ncdr_id_2, attr_name, attr_len)
    ...
    contains
        ...
        subroutine display_1d_attr_string(attr_name)
            character(len=*)                           :: attr_name
            character(len=:), allocatable              :: attr_stor
            
            integer(i_long) :: i
            
            call nc_diag_read_get_global_attr(attr_name, attr_stor)
            
            write (*, "(A, I0, A)") " ** Attribute (1D): " // attr_name // " (Elements: ", len(attr_stor), ")"
